...
- 3.2.0 adds a hidden monkey volume-sound easter egg: four clicks on the BetterTrumpet logo in About unlock and enable an embedded `Assets/monkey.mp3` replacement for the regular tick, then reveals a persistent toggle on the About page. The normal tick remains independently disableable in mouse/volume settings. `MonkeyTickSoundUnlocked`, `UseMonkeyTickSound`, and `UseVolumeTickSound` participate in settings export/import. The long monkey clip is throttled so slider movement does not restart it continuously.
- The post-update changelog is now a compact confirmation window showing the installed version, with `OK` and a localized link to that version's GitHub release. It no longer downloads or renders full release notes inside the app.
